...
def prune_min(t):
    ...
    b = b1 if b1.label < b2.label else b2
    prune_min(b)
    t.branches = [b]        
...
def add_trees(t1, t2):
    ...
    # 我也太吊了
    new_branches = [] 
    for b in zip(t1.branches, t2.branches):         # branches that both have
        new_branches.append(add_trees(b[0], b[1]))
                                                    # one may has more
    l1, l2 = len(t1.branches), len(t2.branches)
    new_branches.extend(t1.branches[l2:l1])         # if l1 <= l2, nothing happens
    new_branches.extend(t2.branches[l1:l2])

...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        extra_cent = max(0, Mint.present_year - self.year - 50)
...
def two_list(vals, amounts):
    ...
    if amounts[0] == 0:
        ...
        new_amounts = amounts[:]
        new_amounts[0] -= 1
        return Link(vals[0], two_list(vals, new_amounts))
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            Fib_next.value = self.value + self.pre.value
        return Fib_next
...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def bst_max_or_min(f):
        assert f == min or f == max
        def func(t):
            if t.is_leaf():
                return t.label
            else:
                return f(t.label, f( [func(b) for b in t.branches] ))
        return func
    bst_max = bst_max_or_min(max)
    bst_min = bst_max_or_min(min)

...
def balanced(m):
    ...
    self_balanced = ( total_weight(end_left) * length(left(m)) == total_weight(end_right) * length(right(m)) )
    return self_balanced and balanced(end_left) and balanced(end_right) 
...
def sub_interval(x, y):
    ...
    "*** YOUR CODE HERE ***"
    # eg: (1, 3) - (5, 6) = (1 - 6, 3 - 5)
    temp_interval = interval(-1 * upper_bound(y), -1 * lower_bound(y))
    return add_interval(x, temp_interval)

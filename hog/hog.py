...
def picky_piggy(score):
    ...
    pig = 142857
    pos_in_pig = score % 6
    if pos_in_pig == 0: pos_in_pig = 6 # also ok when score == 0
    div_num = 10 ** (6 - pos_in_pig) # eg. last digit->//1,first digit->//10^5
    pig = pig // div_num
    return pig % 10
    # END PROBLEM 2
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        this_gain = score - last_score
        if this_gain > running_high:
            print(this_gain, "point(s)! That's a record gain for Player", who, end='')
            print('!')
            new_high = this_gain

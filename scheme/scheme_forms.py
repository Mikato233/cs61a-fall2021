...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        name, value = item.first, scheme_eval(item.rest.first, env)
...
def do_define_macro(expressions, env):
    ...
    # copy from do_define_form, changed a little
    validate_form(expressions, 2)
    signature = expressions.first
    if isinstance(signature, Pair) and scheme_symbolp(signature.first):
        symbol = signature.first
        procedure = MacroProcedure(signature.rest, expressions.rest, env)
        env.define(symbol, procedure)
        return symbol
    else:
        bad_signature = signature.first if isinstance(signature, Pair) else signature

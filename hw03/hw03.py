...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def pong(value, dv, index):                        # change dv when there's an 8 in k
        if index == n:
            return value
        if num_eights(index) != 0 or index % 8 == 0:
            return pong(value - dv, -dv, index + 1)
        else:
            return pong(value + dv, dv, index + 1)
    return pong(1, 1, 1)
...
def missing_digits(n):
    ...
    if n < 10:
        return 0
    def diff_low_2_digits(t):
        print("DEBUG:", t)
        if t >= 10 and (t%10 - (t//10)%10) > 1:
            return t%10 - (t//10)%10 - 1
        else:
            return 0
    return diff_low_2_digits(n) + missing_digits(n // 10)
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    # refer to count_partitions()!!!
    def count_helper(coins, max_coin):
        if coins == 0 or max_coin == 1:             # avoid descending_coin(1)
            return 1
        if coins < 0:
            return 0
        else:
            return count_helper(coins - max_coin, max_coin) + count_helper(coins, descending_coin(max_coin))
